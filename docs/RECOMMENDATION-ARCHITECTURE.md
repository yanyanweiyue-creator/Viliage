...
The Journey workflow uses the same database and scoring contract to build exactly three independent searches: Learn, Advocate, and Connect. Each preserves category and life-stage filters and may use reviewed diagnosis, accommodations, support needs, and insurance signals. If no relevant database row passes the Description Gate, the step remains empty rather than linking an unrelated resource.

## Imported record privacy boundary

`POST /api/profile/documents/scan` accepts a base64 data URL for a supported image or document up to 5 MB. The backend validates the MIME type and size, calls the OpenAI Responses API with `store: false`, and requests strict structured output. The extraction prompt prohibits names, birth dates, addresses, membership/policy/claim numbers, signatures, and provider license numbers. Neither the original data URL nor its bytes are stored in the user profile, D1, local JSON, or the user Google Sheet.

Only user-reviewed extracted fields contribute to later matching. `PATCH /api/profile/documents/:id` lets the user correct fields or disable them by clearing the review flag; `DELETE` removes the metadata and extracted values. The model may extract only facts explicitly present in the upload and must not infer a diagnosis.
