...
const i18n = {
  en: {
    ...
    seasonalEasterEggs: "Show seasonal Easter eggs", seasonalEasterEggsOn: "On", seasonalEasterEggsOff: "Off", seasonalEasterEggsHint: "Show autumn leaves, spring petals, and winter decorations. Off by default; low-stimulation mode keeps them hidden.",
  ...
  zh: {
    ...
    seasonalEasterEggs: "显示季节彩蛋", seasonalEasterEggsOn: "已开启", seasonalEasterEggsOff: "已关闭", seasonalEasterEggsHint: "显示秋日落叶、春日花瓣和冬日装饰。默认关闭；低刺激模式下保持隐藏。",
  ...
  es: {
    ...
    seasonalEasterEggs: "Mostrar sorpresas de temporada", seasonalEasterEggsOn: "Activado", seasonalEasterEggsOff: "Desactivado", seasonalEasterEggsHint: "Muestra hojas de otoño, pétalos de primavera y adornos de invierno. Desactivado por defecto; el modo de baja estimulación los mantiene ocultos.",
...
function communityUnreadBadge(count, className = "") {
  ...
  return `<span class="community-unread-badge ${className} ${unread ? "" : "hidden"}" data-room-unread aria-label="${unread} unread message${unread === 1 ? "" : "s"}"></span>`;
}
...
function communityOverviewHtml(data, posts = state.communityPosts, activeTab = state.communityTab) {
  ...
  const inbox = `<section><h3>${escapeHtml(t("communityFriendRequests"))}</h3><div class="community-grid">${incoming || `<p class="community-empty">${escapeHtml(t("communityNoFriendRequests"))}</p>`}</div></section><section><h3>${escapeHtml(t("communityGroupInvitations"))}</h3><div class="community-grid">${groupInvites || `<p class="community-empty">${escapeHtml(t("communityNoGroupInvitations"))}</p>`}</div></section>${blocks ? `<section><h3>${escapeHtml(t("communityBlockedUsers"))}</h3><div class="community-grid">${blocks}</div></section>` : ""}`;
  ...
  const requestUnread = Math.max(0, Number(counts.requests || 0));
  ...
  const mainHtml = `<div class="community-shell ${activeTab === "moments" ? "moments-active" : ""}">
    ...
    ${activeTab === "moments" ? "" : `<header class="community-village-header">
      ...
      <button type="button" class="community-bell ${requestUnread ? "has-unread" : ""}" data-action="community-tab" data-community-tab="inbox" title="${escapeHtml(t("communityRequestsTab"))}">${communityNavIcon("inbox")}<span class="sr-only">${escapeHtml(t("communityRequestsTab"))}</span>${requestUnread ? `<b aria-label="${requestUnread} unread request${requestUnread === 1 ? "" : "s"}"></b>` : ""}</button>
    </header>`}
...
async function communityPanel() {
  ...
  try {
    ...
    const [feed, documents, stickers, saved] = await Promise.all([
...
async function communityAction(element, action) {
  try {
    ...
    if (action === "community-tab") {
      ...
      if (tab === "moments") {
        await markCommunityKindsRead(["moment", "moment-comment"], tab);
        return openCommunityProfile(state.user?.id);
      }
      state.communityTab = tab;
      state.communityDirectoryOpen = false;
...
function settingsPanel() {
  ...
  openPanel({
    ...
    html: `<p class="panel-intro">${escapeHtml(t("settingsIntro"))}</p>
      ...
      <div class="setting-group seasonal-easter-eggs-settings"><strong id="seasonal-easter-eggs-label">${escapeHtml(t("seasonalEasterEggs"))}</strong>
        <button type="button" role="switch" aria-checked="${String(current.seasonalEasterEggs === true)}" aria-labelledby="seasonal-easter-eggs-label" aria-describedby="seasonal-easter-eggs-hint" class="setting-option seasonal-easter-eggs-toggle ${current.seasonalEasterEggs === true ? "active" : ""}" data-action="toggle-seasonal-easter-eggs">
          <span class="toggle-indicator" aria-hidden="true"></span><span>${escapeHtml(t(current.seasonalEasterEggs === true ? "seasonalEasterEggsOn" : "seasonalEasterEggsOff"))}</span>
        </button>
        <small id="seasonal-easter-eggs-hint">${escapeHtml(t("seasonalEasterEggsHint"))}</small>
      </div>
...
function applySettings() {
  ...
  document.body.classList.toggle("seasonal-easter-eggs-enabled", state.settings.seasonalEasterEggs === true);
...
document.addEventListener("click", (event) => {
  ...
  if (action === "toggle-seasonal-easter-eggs") {
    const scrollTop = $("#panel").scrollTop;
    updateSetting("seasonalEasterEggs", state.settings.seasonalEasterEggs !== true);
    $("#panel").scrollTop = scrollTop;
    $('[data-action="toggle-seasonal-easter-eggs"]')?.focus({ preventScroll: true });
  }
