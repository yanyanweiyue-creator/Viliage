...
The same Apps Script also handles hourly `record-user-count` updates for the `User Count` tab. Set `USER_COUNT_SHEET_WEBHOOK_URL` to the redeployed `/exec` URL and keep `USER_COUNT_SHEET_GID=1958570867`. Each day it creates one row with numeric values under the current row-1 topics: guest sessions, accounts created, searches completed, and average recommendation usefulness. It does not add a date column.
