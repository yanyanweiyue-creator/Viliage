...
The same Apps Script also handles hourly `record-user-count` updates for the `User Count` tab. Set `USER_COUNT_SHEET_WEBHOOK_URL` to the redeployed `/exec` URL, keep `USER_COUNT_SHEET_GID=1958570867`, and the sheet will fill only the four numeric cells under the existing row-1 topics for the current day.
