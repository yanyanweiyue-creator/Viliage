...
Weather ambience and two original fallback scores—daytime **Garden Footsteps** and nighttime **Starlit Current**—are synthesized locally with the browser Web Audio API, and three small licensed animal recordings are served with the site. Bird and gull calls use a quieter original airy voice with slow fades and a short echo instead of an abrupt recording. Browsers require a user gesture, therefore sound starts only after the user selects **Enable sound** in Settings. Master, weather/environment, background-music, and animal volumes are saved on the device; environment defaults louder than the gentler music and animal channels. Low-stimulation mode reduces every channel.
