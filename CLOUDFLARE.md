...
5. Add server-only secrets. Paste each value only into Wrangler's secure prompt:
   ...
   Meeting audio, video, and screen sharing require a TURN relay in production. Create a Cloudflare TURN key and store its ID and API token in `CLOUDFLARE_TURN_KEY_ID` and `CLOUDFLARE_TURN_API_TOKEN`. The Worker keeps the long-lived token server-side and returns only per-member, short-lived ICE credentials. A self-hosted TURN service can instead be stored with `npx wrangler@4 secret put MEETING_ICE_SERVERS_JSON`; its ICE URLs and credentials are returned only to authorized Meeting participants, so use short-lived or tightly restricted TURN credentials. The deployment workflow stops before publishing when neither relay option is configured.
...
- Add both `CLOUDFLARE_TURN_KEY_ID` and `CLOUDFLARE_TURN_API_TOKEN` for Meeting connectivity across NATs and firewalls. Alternatively, add `MEETING_ICE_SERVERS_JSON` for another trusted TURN service. The workflow preserves existing Worker secrets, uploads repository values when supplied, and fails the deployment when no relay secret is present.
